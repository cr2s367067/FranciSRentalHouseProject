...
public extension CS.BigUInt {
    ...
    ///
    /// - Complexity: O(count)
    func multiplied(byWord y: Word) -> CS.BigUInt {
        var r = self
        r.multiply(byWord: y)
        return r
    }
    ...
    /// - Complexity: O(count)
    mutating func multiplyAndAdd(_ x: CS.BigUInt, _ y: Word, shiftedBy shift: Int = 0) {
    ...
    func multiplied(by y: CS.BigUInt) -> CS.BigUInt {
