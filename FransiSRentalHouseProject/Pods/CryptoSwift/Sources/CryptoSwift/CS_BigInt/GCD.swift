...
public extension CS.BigUInt {
    ...
    func inverse(_ modulus: CS.BigUInt) -> CS.BigUInt? {
...
public extension CS.BigInt {
    ...
    func greatestCommonDivisor(with b: CS.BigInt) -> CS.BigInt {
        return CS.BigInt(magnitude.greatestCommonDivisor(with: b.magnitude))
    }
    ...
    /// - Complexity: O(count^3)
    func inverse(_ modulus: CS.BigInt) -> CS.BigInt? {
        guard let inv = magnitude.inverse(modulus.magnitude) else { return nil }
        return CS.BigInt(sign == .plus || inv.isZero ? inv : modulus.magnitude - inv)
    }
}
