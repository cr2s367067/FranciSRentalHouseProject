...
extension Array where Element: FixedWidthInteger {
    ...
    init<Unit: FixedWidthInteger>(count: Int?, generator: () throws -> Unit?) rethrows {
        ...
        if Unit.bitWidth == Word.bitWidth {
            ...
            if let count = count {
                reserveCapacity(count * wordsPerUnit)
            }
            while let unit = try generator() {
                var shift = Unit.bitWidth - Word.bitWidth
                while shift >= 0 {
                    append(Word(truncatingIfNeeded: unit >> shift))
                    shift -= Word.bitWidth
                }
            }
        } else {
            ...
            if let count = count {
                reserveCapacity((count + unitsPerWord - 1) / unitsPerWord)
            }
            var word: Word = 0
            var c = 0
            while let unit = try generator() {
...
extension CS.BigInt: Codable {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        let units = CS.Units(of: UInt64.self, magnitude.words)
        if units.isEmpty {
