...
public final class Blowfish {
    ...
    private func setupBlockModeWorkers() throws {
        encryptWorker = try blockMode.worker(blockSize: Blowfish.blockSize, cipherOperation: encrypt, encryptionOperation: encrypt)

        if blockMode.options.contains(.useEncryptToDecrypt) {
            decryptWorker = try blockMode.worker(blockSize: Blowfish.blockSize, cipherOperation: encrypt, encryptionOperation: encrypt)
        } else {
            decryptWorker = try blockMode.worker(blockSize: Blowfish.blockSize, cipherOperation: decrypt, encryptionOperation: encrypt)
        }
    }
