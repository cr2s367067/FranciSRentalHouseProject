...
struct ProviderRoomSummitView: View {
    ...
    var body: some View {
            ...
            VStack(spacing: 5) {
                ScrollView(.vertical, showsIndicators: false){
                    ...
                    HStack {
                        Spacer()
                        Button {
                            fetchFirestore.summitRoomInfo(
                                uidPath: fetchFirestore.getUID(),
                                id: firebaseStorageDM.imageUIDHolder,
                                holderName: holderName,
                                mobileNumber: holderMobileNumber,
                                roomAddress: roomAddress,
                                town: town,
                                city: city,
                                zipCode: zipCode,
                                emailAddress: holderEmailAddress,
                                roomArea: roomArea)
                        } label: {
                            Text("Next")
                                .foregroundColor(.white)
        ...
        .onAppear(perform: {
            firebaseStorageDM.imageUIDHolder = firebaseStorageDM.imgUIDGenerator()
        })
        .sheet(isPresented: $showSheet) {
            if !image.isSymbolImage {
                firebaseStorageDM.uploadRoomImage(uidPath: fetchFirestore.getUID(), image: image, imgUID: firebaseStorageDM.imageUIDHolder)
