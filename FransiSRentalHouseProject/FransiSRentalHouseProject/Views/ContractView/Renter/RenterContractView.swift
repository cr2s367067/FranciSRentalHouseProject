...
struct RenterContractView: View {
    ...
    var body: some View {
        ZStack {
            ...
            idfEditMode(showEditMode: showEditMode)
        }
        .navigationTitle("")
...
extension RenterContractView {
    ...
    @ViewBuilder
    func idfEditMode(showEditMode: Bool) -> some View {
    ...
    func firstParagraphAndFirstSubThenOne() -> some View {
        ...
        VStack(alignment: .leading, spacing: 5) {
            ...
            VStack(spacing: 6) {
                Group {
                    Text("(一)門牌\(roomsData.zipCode)\(roomsData.city)縣(市)\(roomsData.town)鄉(鎮、市、 區)\(roomsData.roomAddress)(基地坐落__段__小段__地號)。")
...
/*
 var docId: String = ""
 var isSummitContract: Bool = false
 
 //MARK: Contract's Data Model
 var contractBuildDate: Date = Date()
 ...
 var hasParkinglotYes: Bool = false //車位-有
 var hasParkinglotNo: Bool = false //車位-無
 var parkinglotAmount: String = "" //汽機車車位數量
 
 var isSettingTheRightForThirdPersonYes: Bool = false //設定他項權利-有
 var isSettingTheRightForThirdPersonNo: Bool = false //設定他項權利-無
 var SettingTheRightForThirdPersonForWhatKind: String = "" //權利種類
 
 var isBlockByBankYes: Bool = false //查封登記-有
 var isBlockByBankNo: Bool = false //查封登記-無
 
 // MARK: 第一條 委託管理標的 - 租賃範圍
 var provideForAll: Bool = false //租賃住宅全部
 var provideForPart: Bool = false //租賃住宅部分
