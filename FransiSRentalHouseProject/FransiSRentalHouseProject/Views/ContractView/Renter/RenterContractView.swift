...
struct RenterContractView: View {
    ...
    @EnvironmentObject var rentalContractViewModel: RentalContractViewModel
    @EnvironmentObject var firestoreToFetchUserinfo:  FirestoreToFetchUserinfo
                ...
                ScrollView(.vertical, showsIndicators: true) {
                    Group {
                        VStack(spacing: 2) {
                            ...
                            Group {
                                ...
                                SpacerAtRightOfText(contain: "本契約於\(contractBuildDate)經承租人攜回審閱\(contractReviewDays)日（契約審閱期間至少三日）")
                                SpacerAtRightOfText(contain: "承租人簽章：", optionContain: renterSignurture)
                                SpacerAtRightOfText(contain: "出租人簽章：", optionContain: providerSignurture)
                            }
                            Text("房屋租賃契約書")
                                .font(.title2)
                                .font(.system(size: 15))
                        ...
                        .padding(.top, 5)
                        VStack {
                            Text("\t立契約書人承租人__，出租人\(companyTitle)【為□所有權人□轉租人(應提示經原所有權人同意轉租之證明文件)】茲為房屋租賃事宜，雙方同意本契約條款如下：")
                    ...
                    Group {
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            ...
                            VStack(spacing: 6) {
                                Group {
                                    Text("(一)門牌\(roomZipCode)\(roomCity)縣(市)\(roomTown)鄉(鎮、市、 區)\(roomAddress)(基地坐落__段__小段__地號)。")
                                    LineWithSpacer(contain: "(二)專有部分建號\(specificBuildingNumber)，權利範圍\(specificBuildingRightRange)，面積共計\(specificBuildingArea)平方公尺。")
                                    LineWithSpacer(contain: "1.主建物面積：")
                                    Text("\(mainBuildArea)層 平方公尺，__層__平方公尺，__層__平方公尺共計__平方公尺，用途\(mainBuildingPurpose)。")
...
//struct RenterContractView_Previews: PreviewProvider {
...
extension RenterContractView {
    @ViewBuilder
    func contractPresenting() -> some View {
        VStack {
            ScrollView(.vertical, showsIndicators: true) {
                Group {
                    VStack(spacing: 2) {
                        ...
                        Group {
                            SpacerAtRightOfText(contain: "契約審閱權")
                            SpacerAtRightOfText(contain: "本契約於__年__月__日經承租人攜回審閱__日（契約審閱期間至少三日）")
                            SpacerAtRightOfText(contain: "承租人簽章：", optionContain: "")
                            SpacerAtRightOfText(contain: "出租人簽章：", optionContain: "")
                        }
                        Text("房屋租賃契約書")
                            .font(.title2)
                            .font(.system(size: 15))
                    ...
                    .padding(.top, 5)
                    VStack {
                        Text("\t立契約書人承租人\(firestoreToFetchUserinfo.presentUserName())              ，出租人\(rentalContractViewModel.companyTitle)【為□所有權人□轉租人(應提示經原所有權人同意轉租之證明文件)】茲為房屋租賃事宜，雙方同意本契約條款如下：")
                            .font(.system(size: 15, weight: .regular))
                    }
                    .padding(.top, 5)
                    .padding(.horizontal)
                ...
                Group {
                    ...
                    VStack(alignment: .leading, spacing: 5) {
                        ...
                        VStack(spacing: 6) {
                            Group {
                                Text("(一)門牌\(appViewModel.roomZipCode)\(appViewModel.roomCity)縣(市)\(appViewModel.roomTown)鄉(鎮、市、 區)\(appViewModel.roomAddress)(基地坐落__段__小段__地號)。")
