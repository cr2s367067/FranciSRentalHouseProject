...
//        product config: ProductDM,
        ...
        let orderRef = db.collection("users").document(uidPath).collection("ProductOrdered").document(order.orderUID)
        ...
        let shippingRef = db.collection("Stores").document(item.providerUID).collection("ShippingList").document(order.orderUID)
    ...
    func userToSummitProductComment(
        product comment: ProductCommentRatting
    ) async throws {
        let commentRef = db.collection("ProductsPublic").document(comment.productUID).collection("CommentAndRatting")
...
extension FirestoreForProducts {
    ...
    func updateShippingStatus(
        ...
        let userOrderRef = db.collection("users").document(userUID).collection("ProductOrdered").document(orderUID)
...
extension FirestoreForProducts {
    ...
    func userCancelOrder(
        ...
        let shippingRef = db.collection("Stores").document(providerUID).collection("ShippingList").document(orderID).collection("ListContain").document(productUID)
        ...
        let userOrderRef = db.collection("users").document(customerUID).collection("ProductOrdered").document(orderID).collection("ProductList").document(productUID)
        try await userOrderRef.updateData([
            "shippingStatus": shippingStatus.rawValue,
        ])
